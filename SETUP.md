...
Commit them to the default branch. The queue is now initialized.
...
- Allowlist: git add, commit, push, checkout, branch, and your test
  runner.
- Denylist: git push --force, rm -rf, and anything piping a download to
  a shell.

Branch-only blast radius plus this allowlist means the worst case is a
bad branch you never merge.

## 5. First run: "next"

Write one small WP (copy WP-000's shape, set `status: ready`). In the
lane, say "next". Watch it claim, branch, execute, append RESULT, flip
to review, and push, exactly once. Confirm the mechanics before you stop
watching.
